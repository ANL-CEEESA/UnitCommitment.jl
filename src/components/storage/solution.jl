...
function _store_storage_summary!(sol::OrderedDict, sc, T::Int)
    ...
    # Total cost
    summary["Storage: Total cost (\$)"] = _total(charge_cost) + _total(discharge_cost)
    ...
    if haskey(sol, "Storage: Investment status")
        ...
        if !isempty(invest_units)
            summary["Storage: Total investment cost (\$)"] = sum(
                invest_status[su.name] * su.invest for su in invest_units
            )
            summary["Storage: Units invested"] = count(
                su -> invest_status[su.name] > 0.5,
                invest_units,
            )
        end
    end
