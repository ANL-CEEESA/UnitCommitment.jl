...
@testfunction components_interface_build_sf_test begin
    ...
    @test_constr model[:eq_interface_flow_def]["s1", "ifc1", 1] "ni[s1,b2,1] + ni[s1,b3,1] + ni[s1,b4,1] + ni[s1,b5,1] + ni[s1,b6,1] + ni[s1,b7,1] + ni[s1,b8,1] + ni[s1,b9,1] + ni[s1,b10,1] + ni[s1,b11,1] + ni[s1,b12,1] + ni[s1,b13,1] + ni[s1,b14,1] + interface_flow[s1,ifc1,1] = 5.0"

    # ifc2: weighted combination of ISFs for l3 (w=1) and l7 (w=-1)
    @test_constr model[:eq_interface_flow_def]["s1", "ifc2", 1] "0.053 ni[s1,b2,1] + 0.839 ni[s1,b3,1] + 0.655 ni[s1,b4,1] - 0.199 ni[s1,b5,1] + 0.092 ni[s1,b6,1] + 0.502 ni[s1,b7,1] + 0.502 ni[s1,b8,1] + 0.421 ni[s1,b9,1] + 0.363 ni[s1,b10,1] + 0.229 ni[s1,b11,1] + 0.121 ni[s1,b12,1] + 0.138 ni[s1,b13,1] + 0.297 ni[s1,b14,1] + interface_flow[s1,ifc2,1] = 2.106"
    ...
    @test_constr model[:eq_interface_flow_ub]["s1", "ifc2", 1] "interface_flow[s1,ifc2,1] - interface_overflow[s1,ifc2,1] ≤ 55"
end
