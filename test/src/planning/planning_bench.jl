...
function _solve_planning(
    ...
    cand_gens =
        count(u -> u.invest[1] > 1e-5, sc.thermal_units) +
...
function model_planning_test_bench_table()
    @testset "planning_benchmark" begin
        ...
        open("/tmp/planning_bench.csv", "w") do f
            println(
                f,
                "Instance,SolveTime,OptimalCost,CandidateLines,CandidateGens,LinesBuilt,ProfiledsBuilt,ThermalsBuilt,Vars,Constrs,IsStochastic",
            )
            for r in results
                println(
                    f,
                    "$(r.Instance),$(r.SolveTime),$(r.OptimalCost),$(r.CandidateLines),$(r.CandidateGens),$(r.LinesBuilt),$(r.ProfiledsBuilt),$(r.ThermalsBuilt),$(r.Vars),$(r.Constrs),$(r.IsStochastic)",
                )
            end
        end
