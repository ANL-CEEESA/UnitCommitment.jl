...
@testfunction transmission_shiftfactors_contingency_flow_test begin
    for lazy in [false, true]
        base_flow, cont_flows = _solve_shiftfactors("case14/contingency.json", lazy)
        ...
        @test round.(base_flow["l7"], digits = 1) == [-31.3, -26.8, -23.4, -22.4]
        @test round.(base_flow["l14"], digits = 1) == [-56.7, -33.0, -33.0, -33.0]
        ...
        @test round.(cont_flow["l7"], digits = 1) == [-27.2, -22.3, -18.8, -17.8]
        @test round.(cont_flow["l14"], digits = 1) == [-56.7, -33.0, -33.0, -33.0]
...
@testfunction transmission_shiftfactors_congested_flow_test begin
    for lazy in [false, true]
        ...
        @test round.(base_flow["l7"], digits = 1) == [-35.1, -27.4, -24.7, -23.8]
        @test round.(base_flow["l14"], digits = 1) == [-46.8, -33.0, -33.0, -33.0]
