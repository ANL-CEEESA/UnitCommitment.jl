...
function _validate_dc_opf(
    ...
    # Clear finite angle difference limits when the formulation cannot
    # enforce them (e.g. shift factors). In these test cases the limits
    # are non-binding, so the optimal solution is identical.
    if !check_angles
        for sc in instance.scenarios
            for branch in sc[:branches]
                branch.angle_diff_min = -Inf
                branch.angle_diff_max = Inf
            end
        end
    end
    ...
    # Bus voltage angles (only for formulations with theta variables)
    if check_angles
