...
function _lmp_conventional_solve(path::String)
    instance = UnitCommitment.read(path) # no extensions needed
    model = UnitCommitment.build_model(instance, optimizer = test_optimizer())
    optimize!(model)
    return solution(model)
end

@testfunction lmp_conventional_test begin
    ...
    sol = _lmp_conventional_solve(path)
